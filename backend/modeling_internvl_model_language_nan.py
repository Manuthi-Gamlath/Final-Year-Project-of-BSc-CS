...
class InternVLChatModel(PreTrainedModel):
    ...
    def __init__(self, config: InternVLChatConfig, use_flash_attn=True):
        ...
        for name, module in self.language_model.named_modules():
            if isinstance(module, nn.Linear) and module.weight.shape[0] >= 1024:
                lora_target_modules.append((name, module))

        for name, module in lora_target_modules:
            lora_layer = LoRALinear(module, r=8, alpha=16)
            parent = self.language_model
            sub_names = name.split(".")
            for sub_name in sub_names[:-1]:
                parent = getattr(parent, sub_name)
            setattr(parent, sub_names[-1], lora_layer)
    ...
    def forward(
        ...
        try:
            input_embeds[selected] = vit_embeds.view(-1, C)
        except Exception:
            vit_embeds = vit_embeds.view(-1, C)
