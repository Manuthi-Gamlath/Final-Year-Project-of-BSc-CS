...
class InternVLChatModel(PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            #print(flat_labels)
            # Identify float tokens (e.g., "0.33", "0.40")
            vocab = tokenizer.get_vocab()
            float_token_ids = set(vocab.get(f"{x:.2f}") for x in np.arange(0.0, 1.01, 0.01) if f"{x:.2f}" in vocab)
            
            weights = torch.ones_like(flat_labels, dtype=torch.float)
            for tid in float_token_ids:
                weights[flat_labels == tid] = 4.0  # increase weight for numeric values
            #print("weights",weights)
            per_token_loss = loss_fct(flat_logits, flat_labels)
            #print(per_token_loss)
            masked_loss = per_token_loss * (flat_labels != ignore_index)
           # print(masked_loss)
            weighted_loss = (per_token_loss * weights * (flat_labels != ignore_index)).sum()
            #print(weighted_loss)
            norm = (flat_labels != ignore_index).sum()
        
            loss = weighted_loss / norm
