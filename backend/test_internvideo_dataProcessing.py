...
def preprocess_internlm(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            print('sssssssssssssss',conv.get_prompt())
        conversations.append(conv.get_prompt())
        #conversations.append( tokenizer.eos_token)
    ...
    if not text_only:
        ...
        for conversation in conversations:
            #print('ttt',conversation)
            for i in range(num_image):
                image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * num_image_token_list[i]}{IMG_END_TOKEN}'
                conversation = conversation.replace('<image>', image_tokens, 1)
                #print('ttt',image_tokens)
    ...
    input_ids = tokenizer(
        conversations,
        return_tensors='pt',
        padding=True,# if group_by_length or use_packed_ds else 'max_length',
...
query_template ="expert and novice are interacting in provided two video segments concataneated as one video, Eight frames from each segment are sampled from 60 frames, and the task is to predict the engagement values of the expert in the video for all sampled frames, based on social signals of both expert and novice. "

label=["sssssssssssssssss","aaaaaaaaaaaaddddddddddddddddddddddddddddddddddddddddddddddddddaaaaaa"]
label=["dhanuja"]
new_list=[]
for i in label:
...
ret = preprocess_internlm('internvl2_5', new_list,
                                      ...
                                      ds_name="", num_image=num_patches, model_max_length=1000)
...
active_mask = labels != IGNORE_TOKEN_ID
#print(labels*active_mask)
decoded_preds = tokenizer.batch_decode(labels*active_mask, skip_special_tokens=False)
#print(decoded_preds)
    #print(labels)
