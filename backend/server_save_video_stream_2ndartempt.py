...
- understand the user's visible behavior or gesture from the video
- understand the meaning and tone of the user utterance
- choose avatar_emotion so it is suitable for the assistant's response, socially appropriate, and compatible with the user's emotion
- choose mixamo_gesture so it is suitable for the assistant's response, compatible with the user's visible behavior, and also compatible with what the user is saying

Rules:
- JSON only
- no markdown
- no extra text
- avatar_emotion must be appropriate for an assistant avatar, not just a copy of the user's facial emotion
- avatar_emotion should respond naturally to the user's emotional state and spoken content
- mixamo_gesture must be an assistant-side response gesture, not just the user's exact gesture
- mixamo_gesture must fit both the user's behavior and the meaning of the utterance
...
async def process_saved_video_with_live_events(saved_path: str, ws: WebSocket, user_id: str = "user"):
    ...
    try:
        await ws.send_json({"type": "status", "stage": "extracting_video"})
        t = now()
        muted_video_path = extract_uniform_sampled_video_opencv(saved_path, DERIV_DIR, n_frames=16)
        timings["extract_muted_video_ms"] = elapsed_ms(t)

        await ws.send_json({"type": "status", "stage": "extracting_audio"})
        t = now()
        audio_path = extract_audio(saved_path, DERIV_DIR, sr=16000)
        timings["extract_audio_ms"] = elapsed_ms(t)

        await ws.send_json({"type": "status", "stage": "transcribing_audio"})
        t = now()
        text = transcribe_audio(audio_path)
        timings["transcribe_audio_ms"] = elapsed_ms(t)
        ...
        await ws.send_json({"type": "status", "stage": "generating_meta"})
        t = now()
        meta_output = await asyncio.to_thread(
            generate_text_sync,
            muted_video_path,
            build_meta_question(text),
            16
        )
        timings["meta_generation_ms"] = elapsed_ms(t)

        meta = parse_meta_json_response(meta_output)
        await ws.send_json({"type": "meta", "data": meta})

        await ws.send_json({"type": "status", "stage": "streaming_reply"})
        t = now()

        reply_stream = generate_stream_sync(
            muted_video_path,
            build_reply_question(text, meta["avatar_emotion"], meta["mixamo_gesture"]),
            16
        )
        ...
        while True:
            chunk = await asyncio.to_thread(next_chunk_or_none, reply_stream)
            if chunk is None:
                break
