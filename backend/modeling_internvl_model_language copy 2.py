...
class InternVLChatModel(PreTrainedModel):
    ...
    def forward(
        ...
        loss = None
        #print('logits',logits)
        #print('tttttttttttttttt',labels.shape , logits.shape[:2])
        #print(labels,labels.shape)
        ignore_index=-100
        if labels is not None:
            # Ensure labels are padded to match logits
            if labels.shape != logits.shape[:2]:
                labels = torch.nn.functional.pad(labels, (0, logits.shape[1] - labels.shape[1]), value=ignore_index)  # ignore_index = 2
