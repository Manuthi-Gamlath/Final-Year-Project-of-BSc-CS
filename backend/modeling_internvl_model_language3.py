...
class InternVLChatModel(PreTrainedModel):
    ...
    def __init__(self, config: InternVLChatConfig, use_flash_attn=True):
        ...
        lora_config = LoraConfig(
            r=8,
            lora_alpha=32,
            target_modules= ["wo", "wqkv", "w1", "w2","output"],#attention_proj_modules,#"wo", "w1", "w2", "w3",
            lora_dropout=0.1,
            bias="none",
    ...
    def forward(
        ...
        if labels is not None:
            ...
            #shift_weights = attention_mask[..., 1:].contiguous() if attention_mask is not None else None
            #print("shift_labels",labels.shape,shift_labels.shape)
            #print("shift_logits",logits.shape,shift_logits.shape)
            # Flatten tensors for loss computation
            shift_logits = shift_logits.view(-1, self.language_model.config.vocab_size)  # [batch * (seq_len - 1), vocab]
            ...
            per_token_loss = loss_fct(shift_logits, shift_labels)  # [batch * (seq_len - 1)]
            #print("per_token_loss",per_token_loss)
            # Mask out the ignore_index positions
            active_mask = shift_labels != ignore_index
            #active_mask2 = labels != ignore_index
            #print(tokenizer.batch_decode(shift_labels*active_mask, skip_special_tokens=False))
            #per_token_loss = per_token_loss * active_mask
            #print("per_token_loss",per_token_loss)
            #if shift_weights is not None:
            weighted_loss = per_token_loss.sum()
            #else:
            #    weighted_loss = per_token_loss.sum()
        
            norm = active_mask.sum()
            loss = weighted_loss / norm.clamp(min=1)  # avoid division by zero
